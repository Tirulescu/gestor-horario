...
import {
  ArrowUp, ArrowDown, Trash2, Plus, Save, Inbox, X, CalendarClock, BookOpen, Pencil,
} from "lucide-react";
import {
...
import { fmtHour, fmtRange, hourOptions } from "@/lib/hours";
import { getSlotHourSets, normalizeRanges, snapSlotHours, validateSlotRequest, type TimeRange } from "@/lib/studentAvailability";

interface Subject { id: number; name: string; teacherId: number; defaultDurationMin: number; teacher?: { name: string }; }
...
function SlotRow({ r, ri, total, reqs, busy, moveSlot, openEdit, setConfirmTarget, m }: {
  r: SlotRequest; ri: number; total: number; reqs: SlotRequest[]; busy: boolean;
  ...
  return (
    ...
    >
      ...
      <Button size="iconSm" variant="destructive" onClick={() => setConfirmTarget({ kind: "slot", id: r.id, label: `¿Borrar posibilidad ${DAYS[r.dayOfWeek]} ${fmtRange(r.startHour, r.endHour)} de ${m.student.name}?` })} aria-label="Borrar"><Trash2 size={12} /></Button>
    </Reorder.Item>
  );
}
      ...
      <Card className="p-4 space-y-3">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Badge variant={fulfilled ? "success" : "warn"}>{reqs.length}/{m.slotsRequired}</Badge>
        ...
        <Separator />
        {reqs.length === 0 ? (
          <p className="text-sm text-gray-400">Sin posibilidades</p>
        ) : (
          <Reorder.Group
            axis="y"
            values={reqs}
...
export default function RequestsClient() {
  ...
  const [subjects, setSubjects] = useState<Subject[]>(() => warmData<Subject[]>("/api/subjects") ?? []);
  ...
  function hourSetsForStudent(day: number, studentId: number | null, selectedStart?: string) {
  ...
  const addHourSets = useMemo(
    () => hourSetsForStudent(Number(addDay), addStudentId, addStart),
    [availabilities, addDay, addStudentId, addStart, allStudents]
  );
  ...
  const editHourSets = useMemo(
    () => hourSetsForStudent(Number(editDay), editStudentId, editStart),
    [availabilities, editDay, editStudentId, editStart, allStudents]
  );
  ...
  async function loadSubjects(force = false) {
    const cs = force ? null : warmData<Subject[]>("/api/subjects");
    if (cs !== null && cs.length > 0) {
      setSubjects(cs);
      if (activeSubjectId === null) setActiveSubjectId(cs[0].id);
  ...
  async function loadSubjectData(
    ...
    const cSS = force ? null : warmData<SubjectStudent[]>("/api/subject_students");
    const cSR = force ? null : warmData<SlotRequest[]>("/api/slot_requests");
    const cST = force ? null : warmData<Student[]>("/api/students");
    const cAV = force ? null : warmData<Availability[]>("/api/availabilities");
    if (cSS !== null && cSR !== null && cST !== null && cAV !== null) {
      setMembers(cSS.filter((x) => x.subjectId === subjectId));
      setSlotRequests(cSR.filter((x) => x.subjectId === subjectId));
      setAllStudents(cST);
      setAvailabilities(cAV);
      return;
    }
    if (!silent) setLoadingSubject(true);
    try {
      const [ss, sr, st] = await Promise.all([
        fetch(`/api/subject_students?subjectId=${subjectId}`).then((r) => r.json()) as Promise<SubjectStudent[]>,
        fetch(`/api/slot_requests?subjectId=${subjectId}`).then((r) => r.json()) as Promise<SlotRequest[]>,
        fetch("/api/students").then((r) => r.json()) as Promise<Student[]>,
      ]);
      const av: Availability[] = sub ? await fetch("/api/availabilities").then((r) => r.json() as Promise<Availability[]>) : [];
      setMembers(ss);
      setSlotRequests(sr);
      setAllStudents(st);
      setAvailabilities(av);
      put("/api/subject_students", ss); put("/api/slot_requests", sr); put("/api/students", st); put("/api/availabilities", av);
    } finally {
  ...
  async function moveMember(memberId: number, dir: "up" | "down") {
    ...
    if (activeSubjectId !== null) await loadSubjectData(activeSubjectId, { force: true, silent: true });
  }
  ...
  async function deleteSlot() {
    ...
    invalidate("/api/slot_requests"); toast("success", "Posibilidad borrada");
    if (activeSubjectId !== null) await loadSubjectData(activeSubjectId);
  }
  ...
  }, [addOpen, addStudentId, addDay, availabilities, allStudents]);
  ...
  }, [editOpen, editStudentId, editDay, availabilities, allStudents]);

  // al cambiar dia: si la hora elegida no esta disponible, mover a la primera disponible
  function changeAddDay(v: string) {
    setAddDay(Number(v));
    const { startSet, endSet } = hourSetsForStudent(Number(v), addStudentId, addStart);
    if (!startSet.has(addStart) && startSet.size > 0) setAddStart(Array.from(startSet)[0]);
    const stCur = startSet.has(addStart) ? addStart : (startSet.size > 0 ? Array.from(startSet)[0] : "");
    if (!endSet.has(addEnd) && endSet.size > 0) {
      const after = Array.from(endSet).filter((x) => Number(x) > Number(stCur || "0"));
      setAddEnd(after.length > 0 ? after[0] : Array.from(endSet)[endSet.size - 1]);
    }
  }
  function changeEditDay(v: string) {
    setEditDay(Number(v));
    const { startSet, endSet } = hourSetsForStudent(Number(v), editStudentId, editStart);
    if (!startSet.has(editStart) && startSet.size > 0) setEditStart(Array.from(startSet)[0]);
    const stCur = startSet.has(editStart) ? editStart : (startSet.size > 0 ? Array.from(startSet)[0] : "");
    if (!endSet.has(editEnd) && endSet.size > 0) {
      const after = Array.from(endSet).filter((x) => Number(x) > Number(stCur || "0"));
      setEditEnd(after.length > 0 ? after[0] : Array.from(endSet)[endSet.size - 1]);
    }
  }
  ...
  async function submitAdd() {
    ...
    if (!res.ok) return toast("error", (await res.json().catch(() => ({}))).error || "No se pudo guardar");
    invalidate("/api/slot_requests"); toast("success", "Posibilidad añadida");
    setAddOpen(false);
  ...
  const memberName = (sid: number) => allStudents.find((x) => x.id === sid)?.name ?? `#${sid}`;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Define las preferencias de horario de cada alumno por asignatura."
      />

      {/* Selector de asignaturas (chips) */}
      <div className="flex flex-wrap gap-2">
        {loadingSubjects ? (
          <ChipGroupSkeleton />
        ) : visibleSubjects.length === 0 && (
          <div className="flex flex-wrap items-center gap-3">
            <span className="text-gray-500 text-sm">No hay asignaturas.</span>
            <Button asChild size="sm">
              <Link href="/subjects"><Plus /> Crear asignatura</Link>
            </Button>
          </div>
        )}
      ...
      {activeSubject && (
        <div className="flex items-center gap-2 flex-wrap">
          <Button asChild variant="outline" size="sm">
            <Link href="/dashboard"><CalendarClock /> {activeSubject.teacher?.name ?? "Mi panel"}</Link>
          </Button>
          <span className="text-sm text-gray-500">{activeSubject.defaultDurationMin} min</span>
          <Button asChild variant="outline" size="sm">
            <Link href={`/subjects/${activeSubject.id}`}><BookOpen /> Ver asignatura</Link>
          </Button>
        </div>
      )}
      ...
      {activeSubject && !loadingSubject && sortedMembers.length === 0 && (
        <Card className="p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <Button asChild size="sm">
            <Link href={`/subjects/${activeSubject.id}`}><Plus /> Añadir alumnos</Link>
          </Button>
        </Card>
      )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <Select value={addEnd} onValueChange={setAddEnd}>
                  <SelectTrigger><SelectValue placeholder="Selecciona…" /></SelectTrigger>
                  <SelectContent>
                    {HOURS_END.map((o) => hourItem(o, addHourSets.endSet))}
                  </SelectContent>
                </Select>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
              ...
              <Select value={editEnd} onValueChange={setEditEnd}>
                <SelectTrigger><SelectValue placeholder="Selecciona…" /></SelectTrigger>
                <SelectContent>
                    {HOURS_END.map((o) => hourItem(o, editHourSets.endSet))}
                </SelectContent>
              </Select>
            </div>
          </div>
